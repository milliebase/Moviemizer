...
function App() {
  ...
  React.useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&language=en-US&sort_by=popularity.asc&include_adult=false&page=1&vote_count.gte=800`
    )
      .then((res) => res.json())
      .then((json) => {
  ...
  React.useEffect(() => {
    ...
    if (totalPages !== 0) {
      fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&language=en-US&sort_by=popularity.asc&include_adult=false&page=${randomPageId}&vote_count.gte=800`
      )
        .then((res) => res.json())
        .then((json) => {
  ...
  React.useEffect(() => {
    fetch(`https://api.themoviedb.org/3/configuration?api_key=${apiKey}`)
      ...
      .then((json) => {
        let configObj =
          json.images.secure_base_url + json.images.backdrop_sizes[3];
        setConfig(configObj);
      });
  }, []);
